...
ap.add_argument('--k', type=int, default=8,  help='kernels, dim = k * 10')
...
ap.add_argument('--model', type=str, default='MHGNN', help='model name')
ap.add_argument('--khop_num',type=int, default=2, help='Maximum order of k-hop hypergraphs to construct')
ap.add_argument('--KNNhop',action='store_true', default=False, help='choose include the KNN-based hypergraph branch or not')
ap.add_argument('--knn_nodes',type=int, default=-1, help='Number of nearest neighbors in KNN hyperedge branch. If set to -1, use log2(num_nodes) as default')
args = ap.parse_args() 
print('--------Parameters--------')
print(args)
print('--------------------------')
...
log_token += '-khop%s' % args.khop_num 
log_token += '-knn' if args.KNNhop else ''
...
if args.ablation is None:
    model = MHGNN(args, data_loader)  
else:
    if args.ablation == 'baseline':
        model = baseline(args, data_loader)
    elif args.ablation == 'without_motifs':
        model = without_motifs(args, data_loader)
    elif args.ablation == 'remove_global':
        model = remove_global(args, data_loader)
    elif args.ablation == 'all_gcn':
        model = all_gcn(args, data_loader)
    else:
...
def evaluate_pred(data_loader, data):
    model.eval()
    total = 0.
    n_samples = 0.
    total_loss = 0.
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.mylog:
            ...
            if np.isnan(pcc_states):
                writer.add_scalars('data/pcc_states', {'val': 0}, epoch)
            else:
